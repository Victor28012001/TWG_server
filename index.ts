...
app.get('/api/wallet', authMiddleware, async (req: Request, res: Response) => {
    try {
        ...
        if (walletAddress) {
            tokenBalance = await getUserTokenBalance(walletAddress);
            try {
                const pubkey = new PublicKey(walletAddress);
...
app.post('/api/token/claim-daily', authMiddleware, async (req: Request, res: Response) => {
    try {
        ...
        const lastClaim = userDailyClaims.get(userId) || 0;
        const now = Date.now();
        ...
        const signature = await mintTokensToUser(walletAddress, 100);
        userDailyClaims.set(userId, now);

        res.json({
            success: true,
            reward: 100,
            signature,
            newBalance: await getUserTokenBalance(walletAddress)
        });
    } catch (error) {
...
//         const wallets = await getWalletsWithRetry(userDetails, 3, 2000);
//         const solanaWallet = wallets.find(w => w.type === 'solana');
//         const userWalletAddress = solanaWallet?.walletAddress;
...
//         // Check balance
//         const currentBalance = await getUserTokenBalance(userWalletAddress);
//         if (currentBalance < totalCost) {
//             return res.status(400).json({
//                 error: `Insufficient funds. Need ${totalCost} $JBKS, have ${currentBalance.toFixed(2)}`
//             });
//         }

//         // ✅ Transfer tokens FROM user TO game wallet
//         // Since Civic manages the user's wallet server-side, we use
//         // the game keypair to facilitate — but we deduct from user's
//         // tracked balance and the actual on-chain transfer uses game wallet
//         // In production with Civic embedded wallets, use their SDK to sign
//         const userPubkey = new PublicKey(userWalletAddress);
//         const gamePubkey = gameKeypair!.publicKey;

//         const userTokenAccount = await getOrCreateAssociatedTokenAccount(
//             connection, gameKeypair!, TOKEN_MINT, userPubkey
//         );
//         const gameTokenAccount = await getOrCreateAssociatedTokenAccount(
//             connection, gameKeypair!, TOKEN_MINT, gamePubkey
//         );

//         // Create transfer instruction (game wallet pays fees, user's tokens moved)
//         // NOTE: This requires the game wallet to have authority to transfer user tokens
//         // For Civic embedded wallets, this works because Civic signs server-side
//         const transferIx = createTransferInstruction(
//             userTokenAccount.address,
//             gameTokenAccount.address,
//             userPubkey,
//             BigInt(Math.floor(totalCost * (10 ** 9))),
//         );
...
//         transaction.add(transferIx);
//         transaction.recentBlockhash = blockhash;
//         transaction.feePayer = gamePubkey; // Game pays fees

//         // ✅ Return transaction for client to sign with Civic wallet
...
io.on('connection', (socket: CustomSocket) => {
    ...
    //     console.log(`💰 Purchase request - Player: ${player.name}, Item: ${item.name}, Cost: ${totalCost} JBKS, Has signature: ${!!data.signature}`);

    //     // If this is a free purchase (from ad)
    //     if (data.free) {
    //         console.log("🎁 Processing free purchase from ad");
    //         const result = storeManager.purchaseItem(player, data.itemId, data.quantity || 1);

    //         if (result.success && result.item) {
    //             // Apply the purchased item to player
    //             await applyPurchasedItem(player, result.item, data.quantity);
    ...
    //             if (result.item.price >= 1000) {
    //                 io.to(socket.data.matchId!).emit('serverResponse', {
    //                     type: 'announcement',
    //                     text: `${player.name} just claimed ${result.item.name} from watching an ad!`
    //                 });
    //             }
    //         } else {
    ...
    //     // If no signature, this is the first step - create transaction for client to sign
    //     if (!data.signature) {
    //         try {
    //             console.log(`📝 Creating transfer transaction for ${totalCost} JBKS from ${player.walletAddress}`);

    //             // Verify user has sufficient balance
    //             const currentBalance = await getUserTokenBalance(player.walletAddress);
    //             player.tokenBalance = currentBalance;

    //             console.log(`💰 Current balance: ${currentBalance} JBKS`);

    //             if (currentBalance < totalCost) {
    //                 callback({
    //                     success: false,
    //                     message: `Insufficient funds. Need ${totalCost} JBKS, you have ${currentBalance}`
    //                 });
    //                 return;
    //             }

    //             // Create transfer transaction
    //             const { transaction, userTokenAccount, gameTokenAccount } = await createTransferTransaction(
    ...
    //             callback({ success: false, message: 'Failed to create payment transaction: ' + error.message });
    //         }
    //         return;
    //     }
    ...
    //     try {
    //         console.log(`🔐 Processing signed transaction: ${data.signature}`);

    //         // Confirm transaction
    //         const confirmation = await connection.confirmTransaction(data.signature, 'confirmed');

    //         if (confirmation.value.err) {
    //             console.error(`❌ Transaction failed: ${confirmation.value.err}`);
    //             callback({ success: false, message: 'Transaction failed. Please try again.' });
    //             return;
    //         }
    ...
    //         // Get updated balance
    //         const newBalance = await getUserTokenBalance(player.walletAddress);
    //         const expectedBalance = player.tokenBalance - totalCost;

    //         console.log(`💰 Balance update - Previous: ${player.tokenBalance}, Expected: ${expectedBalance}, New: ${newBalance}`);

    //         // Update player's token balance
    //         player.tokenBalance = newBalance;

    //         // Process the purchase
    //         const result = storeManager.purchaseItem(player, data.itemId, data.quantity || 1);

    //         if (!result.success) {
    //             console.error(`❌ Purchase failed after payment: ${result.message}`);
    //             callback({ success: false, message: 'Payment processed but item delivery failed. Support has been notified.' });
    //             return;
    //         }
    ...
    //             level: player.progression?.data?.level || 1,
    //             headshots: player.progression?.data?.headshots || 0,
    //             highestKillstreak: player.progression?.data?.highestKillstreak || 0,
    //             score: (player.kills || 0) * 100 + (player.progression?.data?.headshots || 0) * 50,
    //             wallet: player.walletAddress,
    //             tokenBalance: player.tokenBalance
    //         });

    //         console.log(`✅ Purchase completed: ${player.name} bought ${result.item.name} for ${totalCost} JBKS`);
    ...
    //         // Broadcast to others if it's a global announcement
    //         if (result.item.price >= 1000) {
    //             io.to(socket.data.matchId!).emit('serverResponse', {
    //                 type: 'announcement',
    //                 text: `${player.name} just bought ${result.item.name} from the store!`
    //             });
    //         }

    //     } catch (error) {
    //         console.error('Error processing purchase:', error);
    //         callback({ success: false, message: 'Purchase failed. Please try again.' });
    //     }
    // });
    ...
    socket.on('store-purchase', async (data: {
        ...
        if (data.free) {
            ...
            // Update database
            await balanceService.addToInventory(player.id || socket.data.userId, item);
        ...
        // Process paid purchase using database balance (NO BLOCKCHAIN TRANSACTION)
        try {
            ...
            console.error('Purchase error:', error);
            callback({ success: false, message: 'Purchase failed' });
        }
    });
    ...
    socket.on('useItem', (data: { itemId: string }) => {
        const match = getMatch(socket);
        const player = socket.data.player;
        if (!match || !player) return;
